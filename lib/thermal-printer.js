...
export function generateESCPOSReceipt(receiptData) {
  ...
  if (payment) {
    commands += '--------------------------------' + ESCPOS.LINE_FEED
    commands += ESCPOS.BOLD_ON
    commands += `Payment: ${payment.method.toUpperCase()}` + ESCPOS.LINE_FEED
    commands += ESCPOS.BOLD_OFF
    if (payment.method === 'cash') {
      commands += `Paid:`.padEnd(24) + `LKR ${payment.amount_paid.toFixed(2)}`.padStart(10) + ESCPOS.LINE_FEED
      commands += `Change:`.padEnd(24) + `LKR ${payment.change.toFixed(2)}`.padStart(10) + ESCPOS.LINE_FEED
    } else {
      commands += `Card Payment: LKR ${payment.amount_paid.toFixed(2)}` + ESCPOS.LINE_FEED
    }
  }
