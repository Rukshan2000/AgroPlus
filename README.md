...
2. **Environment Configuration**
   ...
   DB_PASSWORD=1998
   NEXT_PUBLIC_SUPABASE_URL="postgresql://postgres:1998@127.0.0.1:5432/saas"
...
2. Configure env
Copy `.env.example` to `.env` and fill `NEXT_PUBLIC_SUPABASE_URL`.
...
- Vercel: Use `NEXT_PUBLIC_SUPABASE_URL` from Neon or Supabase. Next.js App Router deploys seamlessly [^1]. 
- Heroku: Add `@neondatabase/serverless` compatible `NEXT_PUBLIC_SUPABASE_URL` (or standard Postgres), set env vars, run:
  \`\`\`
  npm run migrate && npm run seed
  \`\`\`
- Add build and start commands to package.json. Ensure `NEXT_PUBLIC_SUPABASE_URL` is configured.
...
- Missing `NEXT_PUBLIC_SUPABASE_URL`: migrations will fail
